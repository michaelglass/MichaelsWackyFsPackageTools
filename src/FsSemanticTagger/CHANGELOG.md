...
- fix: **the gate between publication waves waits for the dependency to be restorable, not merely indexed, and NuGet is asked about each package once per wave.** The gate cleared as soon as nuget.org's flat container listed the version, but the index lists a version minutes before a restore of it succeeds (measured on FsHotWatch: four minutes for a CLI), and the next wave's nuspec names that exact version. A consumer that ran its own restorability barrier after `release` was then asking the same question twice per wave, once weakly by this poll and once properly by the barrier — up to 40 minutes on one fact.
  - New `Api.checkRestorable`. For a library the `dotnet restore` probe decides alone: the index listing a version does not pass a package the probe cannot fetch, and a privately published package passes without the public index listing it. For a `PackAsTool` package a `PackageReference` probe always fails NU1212, so the flat container decides instead. New `ReleaseInput.CheckRestorable` seam (`isTool -> packageName -> version -> FeedPresence`), separate from `CheckFeedPresence`, whose question ("is it published") the orphan-tag detection and the last wave's confirmation still ask.
  - The contract with a consumer's own barrier: pass `--skip-nuget-wait` when a stronger check follows this command (FsHotWatch's `scripts/wait-for-nuget.fsx` installs and runs the published tool). The inter-wave gate is then the only question asked about a wave with dependents, and the barrier the only one asked about the last wave. The flag never lifts the gate. The README's flag text, publication-order steps and exit-code summary now say which probe each wait uses.
...
- fix: **`release --check` no longer certifies a changelog that promotion does not deliver, and a dependency bump can no longer vanish from a release's changelog**. Releasing `SqlHydra.Query.Pgvector` 0.1.0-alpha.5, `alpha --check` passed on its stated contract of an Unreleased entry "authored or derivable", while promotion copied only the authored block and derived nothing from the other 14 commits. The published changelog omitted the one change a consumer could observe: `SqlHydra.Query` 4.1.0-beta.2 → 4.1.0-beta.3.
...
- fix: **a workflow run that has not appeared YET is no longer reported as a missing trigger.** On 2026-09-16 `release` pushed `v0.1.0-alpha.5`, heard "no run" from GitHub, and exited 1 with `MISSING TRIGGER` — while the Release run registered seconds later and went green; the natural recovery, re-pushing the tag, would have published the version twice.
  ...
  - The second defect found in the same release, the changelog `--check` contract, is not part of this change (fixed in 0.14.0-alpha.11).
...
- fix: **the post-push poll no longer gives up before GitHub has registered the workflow run.** It asked once, three seconds after pushing the tag, and reported the answer. GitHub registers a tag-push run seconds later rather than instantly, so on 2026-09-04 three healthy FsHotWatch releases — core `0.10.0-alpha.28`, Fantomas `0.7.0-alpha.23` and Cli `0.14.0-alpha.42` — were each declared broken while their Release workflows were already running, and each needed a human to work out which half of "the release did not happen / the release happened and the poll gave up" they were looking at. The poll now keeps asking for five minutes (5s x 60), and an unanswerable `gh` is retried like an absent run rather than settled as one.
  ...
  - **Three outcomes, three exit codes**, the convention already used for the NuGet wait, applied one stage earlier: `0` every tag has a run, `1` the release demonstrably did not happen (a push failed, or a run exists and has already finished without publishing), `2` the tags are on the remote and no run has appeared yet. `2` and not `1` is the point — "I stopped waiting" is not "it failed", and the old code returned `1` for both.
