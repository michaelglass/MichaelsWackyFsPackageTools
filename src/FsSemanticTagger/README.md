...
2. The exact version of each of those packages must be **restorable** before any tag of the next wave is pushed. The index lists a version minutes before a restore of it succeeds, and the next wave's nuspec names this exact version, so the gate asks `dotnet restore` (honouring the repo's `nuget.config`), not the index. A `<PackAsTool>` package cannot be probed that way (NU1212), so for a tool the index decides. This is the only NuGet question asked about a wave that has dependents.
...
Exit `0` means every tag has a run, every package with dependents in the release is restorable, and (unless `--skip-nuget-wait`) every package of the last wave is on the index. Exit `1` is reserved for a release that demonstrably did not happen: CI red, a tag push that failed, or a publish run that finished without publishing.
