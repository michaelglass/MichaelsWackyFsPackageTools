...
let private packLocally (run: string -> string -> CommandResult) (bumps: (PackageConfig * Version) list) : int =
    for (pkg, _version) in bumps do
        ...
        // flag the RefStamp guard would refuse to emit a
...
///     analyzer-package layout bug fixed. Driving a REPUBLISH
        ...
        | HasPreviousRelease currentVersion ->
            ...
            | true, _ ->
                ...
                | Auto, true ->
                    ...
                    // CLI change as a patch — the guard silently disabled for
                    // the whole set it was built for.
                    //
...
let private executeBumps
    ...
    // commit descriptions. A hand-authored section always
...
/// its commit descriptions. A pre-flight gate for `mise run ci`
