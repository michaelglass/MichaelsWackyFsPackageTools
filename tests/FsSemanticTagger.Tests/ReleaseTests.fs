...
let ``release - StartAlpha with LocalPublish calls dotnet pack`` () =
    ...
    try
        ...
        // carry the explicit release flag or the RefStamp guard
...
// =============================================================================
// a PackAsTool package must still get its CLI grammar diffed.
//
// correctly stopped constructing an API probe for PackAsTool
// packages (a PackageReference to a tool package fails NU1212). The tracked issue
...
let ``release - PackAsTool grammar break bumps major without constructing an API probe`` () =
    ...
    try
        ...
        test <@ result = 0 @>
        // Before the tracked issue this produced 1.0.1 — a breaking CLI change shipped
...
let ``release - PackAsTool CLI aborts when the previous grammar cannot be read`` () =
    // FAIL CLOSED (option b). The package HAS a CLI grammar — the
